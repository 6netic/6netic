...
def check_planning(request):
    """ Checks submitted values from insert_planning form """
    ...
    if len(error_msg) == 0:
        variables['date_tour'] = escape(request.POST.get("date_tour"))
        variables['nurse1_tour_pref'] = escape(request.POST.get("nurse1_tour_pref"))
        variables['nurse2_tour_vill'] = escape(request.POST.get("nurse2_tour_vill"))
        variables['rec_confirm'] = escape(request.POST.get("rec_confirm"))
        return variables
    else:
        return error_msg
